...
void Simulation::addMemory() {
	if (m_memoryManager.can_add() == false)
		return;

	m_memoryManager.add_memory(m_memoryMaker.get_memory());
}


void Simulation::loadMemory() {
	std::vector<std::vector<float>> memories = m_memoryManager.selected_memories();
	if (memories.size() == 0)
		return;
...
void Simulation::updateAndRender(sf::RenderWindow& window) {
	...
	if (getButton("both", "Neural Net")->getClicked()) {
		...
		m_memoryMaker.draw_border(window);
		m_memoryMaker.draw(window);
	}

	m_memoryManager.draw(window);
}
